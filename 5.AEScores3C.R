...
TableAEP <- join(TableAEP,LackFeed, by = "o9")
...
LocalS$Sum <- ifelse(rowSums(LocalS[,c(3:5)]) > 0, 1, 0) + LocalS$d32
...
# yes = 1 / no = 0
#d17: "Animal manure", "Pulses in association/rotation with main crops",
#"Cover crops in association and/or rotation with main crops"
#"Ramial Wood Chip (RWC) or other wood chips"
#d20: "Crop rotation / intercropping", "Crop mixtures"
#d27: "Crop rotation / intercropping"
Synergy <- rbind(HouseholdCambodia_2C[,c(1,1106,1110,1111,1115,1326,1333,1603)],
                 HouseholdLaos_2C[,c(1,1068,1072,1073,1077,1252,1259,1484)],
                 HouseholdVietnam_2C[,c(1,1114,1118:1119,1123,1298,1305,1517)])
for (i in 2:ncol(Synergy)){
    Synergy[,i] <- as.numeric(Synergy[,i])
    Synergy[,i] <- ifelse(Synergy[,i] == 2, 1, 0)
}
Synergy$Score <- ifelse(rowSums(Synergy[2:8], na.rm = T) > 0, 1, 0)
TableAEP <- join(TableAEP,Synergy[c(1,9)], by = "o9")
...
TableAEP <- join(TableAEP,i2, by = "o9")
...
TableAEP$V10 <- rowSums(TableAEP[,c(41:42)], na.rm = T)
...
colnames(TableAEP)[47] <- "V11_3"

#We create the score for the 11th principle
TableAEP$V11 <- rowSums(TableAEP[,c(45:47)], na.rm = T)
...
colnames(TableAEP)[51] <- "V13_3"

#We create the score for the 13th principle
TableAEP$V13 <- rowSums(TableAEP[,c(49:51)], na.rm = T)


## 3. Final score calculation and average values
Scores <- TableAEP[,c(1:4,7,12,15,19,24,27,31,35,41,44,48,52)]
Scores$V9 <- Scores$V9 / 9
Scores$V10 <- Scores$V10 / 2
Scores$V11 <- Scores$V11 / 4
Scores$V13 <- Scores$V13 / 4
#We calculate average score value for each principle
AvScores <- aggregate(cbind(V9,V10,V11,V13) ~ o5, data = Scores, FUN = mean, na.rm = TRUE)
colnames(AvScores) <- c("Area","9.Participation","10.Connectivity","11.Connectivity",
                        "13.Participation")
rownames(AvScores) <- c("Vietnam-DB","Cambodia","Vietnam-SL","Laos")
AvScores <- AvScores[,c(2:5)]
...
radarchart(AvScores, axistype=1 ,
           #custom polygon
...
# Add a legend
legend(x=0.7, y=1, legend = rownames(AvScores[-c(1,2),]), bty = "n", pch=20 , col=colors_in , text.col = "grey", cex=1.2, pt.cex=3)
