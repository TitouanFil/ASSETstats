...
#Script for all crops 
{
  ...
  HouseholdVietnam_2C$Fruit <- rowSums(HouseholdVietnam_2C[,c(2738,2739,2743,2744,2747,2761,
                                                              ...
                                                              2801,2804,2805,2807,2811,2815,2817:2819)], na.rm = T)
  ...
  #3.12 Macadamia area (ha): should it be included in fruits? Also depending on number of hh growing macadamia
  HouseholdVietnam_2C$Macadamia <- ifelse(is.na(HouseholdVietnam_2C[,2821]),0,HouseholdVietnam_2C[,2821])
}
#Columns 2826:2833
#3.13 Nb of cattle heads
  HouseholdVietnam_2C$Calf <- HouseholdVietnam_2C[,1889]/2
  HouseholdVietnam_2C$Cattle <- rowSums(HouseholdVietnam_2C[,c(1888,2834)], na.rm = T)
  #Column 2835
#3.14 Nb of buffalo heads 
  HouseholdVietnam_2C$CalfBuff <- HouseholdVietnam_2C[,1887]/2
  HouseholdVietnam_2C$Buffalo <- rowSums(HouseholdVietnam_2C[,c(1886,2836)], na.rm = T)
  #Column 2837
#3.15 Nb of goat heads
  HouseholdVietnam_2C$Roe <- HouseholdVietnam_2C[,1893]/2
  HouseholdVietnam_2C$Goat <- rowSums(HouseholdVietnam_2C[,c(1892,2838)], na.rm = T)
  #Column 2839
#3.16 Nb of pig heads
  HouseholdVietnam_2C$Piglet <- HouseholdVietnam_2C[,1891]/2
  HouseholdVietnam_2C$Pig <- rowSums(HouseholdVietnam_2C[,c(1890,2840)], na.rm = T)
  #Column 2841
#3.17 Nb of poultry
...
#3.27 Final Table creation
  VarPCA <- HouseholdVietnam_2C[,c(23,32,59,758,1855,2278,2279,2731,2826:2833,2835,2837,2839,2841:2847)]
...
## 4. PCA
#PCA - Moc Chau
res.pca <- PCA(PCAMC[,c(3:26)], scale.unit = T, ncp = 16, graph = TRUE)
...
for (i in 1:(nrow(contrib)-1)){
  contrib[i,17] <- contrib[i,1]*contrib[25,1]+contrib[i,2]*contrib[25,2]+
    contrib[i,3]*contrib[25,3]+contrib[i,4]*contrib[25,4]+
    contrib[i,5]*contrib[25,5]+contrib[i,6]*contrib[25,6]+
    contrib[i,7]*contrib[25,7]+contrib[i,8]*contrib[25,8]+
    contrib[i,9]*contrib[25,9]+contrib[i,10]*contrib[25,10]+
    contrib[i,11]*contrib[25,11]+contrib[i,12]*contrib[25,12]+
    contrib[i,13]*contrib[25,13]+contrib[i,14]*contrib[25,14]+
    contrib[i,15]*contrib[25,15]+contrib[i,16]*contrib[25,16]
  }
contrib <- contrib[order(contrib$contribTot),]
contrib$sum3 <- rowSums(contrib[,c(1:16)])
...
#PCA - Dien Bien
res.pca <- PCA(PCADB[,c(3:24)], scale.unit = TRUE, ncp = 8, graph = TRUE)
