...
# # #1.2.d. Province Plot for One column results (for HouMember data)
ListBPlotProvHouM <- function(data,title,pond,ylimn,dig){
dcount <- data
dcount$nbHHtot <- ifelse(dcount$'Province' == 'Dien Bien province',
                         sum(HouHead$Province == 'Dien Bien province'),
                         sum(HouHead$Province == 'Son La province'))
dcount$Percentage_of_households <- round(dcount$n/as.numeric(dcount$nbHHtot)*100, digits=0)
#And now a distribution plot
plotb <- ggplot(data=dcount, aes(x= Answer, y=Percentage_of_households, fill = Province))+
  geom_bar(colour="black",stat="identity",position = 'dodge')+
  theme(axis.text.x = element_text(face="bold", size=10,angle = 60))+
  geom_text(aes(label = Percentage_of_households), vjust = -1, position = position_dodge(0.9))+
  ylim(ylimn, max(dcount$Percentage_of_households)+20)+
...
ylimn <- 0
dig <- 0
ListBPlotProvHouM(dcount,title,pond,ylimn,dig)
...
dcount$Answer <- c('Agricultural work\n (own farm)','Other farm\n salaried employment\n (permanent)',
                  'Other farm\n salaried employment\n (temporary)','Government','Business (owner)', 
                  'Salaried employment\n (non-agricultural work)','Housewife/\n caregiver','Unable\n to work','Other',
                  'Agricultural work\n (own farm)','Other farm\n salaried employment\n (permanent)',
                  'Other farm\n salaried employment\n (temporary)','Government','Business (owner)', 
                  'Salaried employment\n (non-agricultural work)','Housewife/\n caregiver','Unable\n to work','Other')
...
#Then we count the frequency of number of household member per province
nhmcount <- p_nocount %>%
  count(`Province`,`dum`)
...
d1dat <- copy_labels(d1dat, HouseholdVietnam_2C)
...
#We create a counting table and a title
CropYNcount <- count(HouseholdVietnam_2C, CropYN)
...
result$AverageTot <- result$d2_132/sum(!is.na(HouseholdVietnam_2C$no_crop1))
...
pond <- sum(!is.na(HouseholdVietnam_2C$no_crop1))
...
result$AverageTot <- result$d2_232/sum(!is.na(HouseholdVietnam_2C$no_crop2))
result$AverageTot <- as.numeric(format(result$AverageTot,scientific = FALSE, digits = 1))
#Now we prepare the data for the function
#We determine other function parameters
